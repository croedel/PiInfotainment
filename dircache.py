...
def update_dir_cache( dir_cache ):
  ...
  for dir_item, val in dir_cache['dir'].items():
    if val['meta'][2] == False: # directory no loger existing    
      val['files'].clear()
      updated = True

  if updated or dir_cache['statistics']['latest_change'] < latest_change:
    logging.info('Directory cache refreshed: {} directories'.format(dir_cnt) )
...
def set_exif_info( file_path_name, orientation, dt, exif_info ):
  ...
  try:
    ...
    logging.warn("Couldn't update EXIF info for: {}".format(file_path_name))

# this is the main function - returs a filtered file list
def get_file_list( dt_from=None, dt_to=None ):
  ...
  for path, val in dir_cache['dir'].items():
    for item, attr in val['files'].items():
      ...
      ftime = attr[2] if attr[2] != None else attr[1] # prefferably use EXIF date, fallback is mdate 
      if ftime > dt_from and ftime < dt_to:
        add_to_filelist = True 
      if config.OUTDATED_FILE_PROP > 0 and random.randint(1,config.OUTDATED_FILE_PROP) == 1:
        add_to_filelist = True   
      if add_to_filelist:
